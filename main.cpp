...
static int writeToUart(src::Drivers *drivers,std::string s){
    ...
    int res = drivers->uart.write(tap::communication::serial::Uart::UartPort::Uart7,(uint8_t*)char_array,n);
    modm::delay_ms(100);
    return res;
}
...
static int writeToUart(src::Drivers *drivers,char* s,int n){
    // declaring character array
    int res = drivers->uart.write(tap::communication::serial::Uart::UartPort::Uart7,(uint8_t*)s,n);
    modm::delay_ms(100);
    return res;
}
...
static int readFromUart(src::Drivers *drivers,char *buffer){
    ...
    bool read_flag = 1;
    while(read_flag){
    int res = drivers->uart.read(tap::communication::serial::Uart::UartPort::Uart7,(uint8_t*)(buffer+bytes_read));
...
int main()
{
    ...
    while (1)
    {
        ...
        // std::string s = std::to_string(drivers->mpu6500.getPitch())+ " "+std::to_string(drivers->mpu6500.getRoll())
        //  +" "+std::to_string(drivers->mpu6500.getYaw())+"\n";
       ...
       // drivers->uart.write(tap::communication::serial::Uart::UartPort::Uart7,(uint8_t*)char_array,6);
        // writeToUart(drivers,"arpan");
        // if(writeToUart(drivers,s)){
        //     modm::delay_ms(100);
        // }
        // else{
        //     string s = "did not write";
        //     writeToUart(drivers,s);
        // }
